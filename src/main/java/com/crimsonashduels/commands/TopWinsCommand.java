...
public class TopWinsCommand implements CommandExecutor {
    ...
    public TopWinsCommand(CrimsonAshDuels plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        List<Map.Entry<String, Integer>> top = stats.getTopWins(5);

        sender.sendMessage("§6--- Top 5 Wins ---");
        int rank = 1;
        for (Map.Entry<String, Integer> entry : top) {
            String name = Bukkit.getOfflinePlayer(java.util.UUID.fromString(entry.getKey())).getName();
            sender.sendMessage(rank + ". " + name + " - " + entry.getValue() + " wins");
            rank++;
        }
        return true;
    }
